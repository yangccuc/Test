#MMG_Pack("MMG_State_Level_Regression_Model_Data.csv", "Avondaledata.csv", "Cincinnati Monthly Unemployment Rate.csv")

MMG_Pack <- function(MMG_State_Level_Regression_Model_Data, Avondaledata, Cincy_Mon_unemp_rate){
...
MMGData<-read.csv(paste(MMG_State_Level_Regression_Model_Data))
...
AvondaleData<-read.csv(paste(Avondaledata))
...
ueData<-read.csv(paste(Cincy_Mon_unemp_rate))
...
return(list(FIAvondale, plot_fn))
}
